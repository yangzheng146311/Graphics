...
Renderer::Renderer(Window &parent) : OGLRenderer(parent) { 
	...
	if(!textures[0] || !textures[1]) { 
	 ...
	 currentShader = new Shader(SHADERDIR"TexturedVertex.glsl",  SHADERDIR"TexturedFragment.glsl"); 
		 ...
		 usingDepth = false; usingAlpha = false; 
		 blendMode = 0; 
		 modifyObject = true; 
...
void Renderer::RenderScene() { 
	...
	glActiveTexture(GL_TEXTURE0);  for (unsigned int i = 0; i < 2; ++i) { 
...
void Renderer::ToggleAlphaBlend() {
	usingAlpha = !usingAlpha;  usingAlpha ? glEnable(GL_BLEND) : glDisable(GL_BLEND);  } 
 void Renderer::ToggleBlendMode() {  blendMode = (blendMode + 1) % 4; 
 switch (blendMode) {  
 ...
 case(3):glBlendFunc(GL_SRC_ALPHA, GL_ONE); 
	 break; 
 }; 
 }
