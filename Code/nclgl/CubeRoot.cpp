...
body->SetModelScale(Vector3(30, 60, 100));
body->SetTransform(Matrix4::Translation(Vector3(3000, 500, 3000)));
AddChild(body);

head = new SceneNode(cube, Vector4(0, 1, 0, 1)); //Green!
head->SetModelScale(Vector3(50, 50, 50));
head->SetTransform(Matrix4::Translation(Vector3(500, 300, 0)));
body->AddChild(head);

leftArm = new SceneNode(cube, Vector4(0, 0, 1, 1)); //Blue!
leftArm->SetModelScale(Vector3(60, -180, 30));
leftArm->SetTransform(Matrix4::Translation(Vector3(-120, 30, -1)));
body->AddChild(leftArm);

rightArm = new SceneNode(cube, Vector4(0, 0, 1, 1)); //Blue!
rightArm->SetModelScale(Vector3(90, -180, 30));
rightArm->SetTransform(Matrix4::Translation(Vector3(120, 30, -1)));
